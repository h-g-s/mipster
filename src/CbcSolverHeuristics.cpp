      ...
      for (iColumn = 0; iColumn < numberColumns; iColumn++) {
        ...
        if (state[iColumn] == 1) {
          ...
          if (djValue < -1.0e-2 || otherValue < -1.0e-2) {
            // printf("XX dj of %d at %g is %g - %d out of %d contribute
            // %g\n",iColumn,value,djValue,
            ...
            // printf("dj of %d at %g is %g - no contribution from
            // %d\n",iColumn,value,djValue,
  ...
  int noPrinting_, int initialPumpTune)
{
  ...
  int useNoRelRepair = parameters[CbcParam::NORELREPAIR]->modeVal();
  ...
  if (useNoRelRepair >= kType && useNoRelRepair <= kType + 1) {
    ...
    model->addHeuristic(&heuristicNoRel);
    anyToDo = true;
  }
  // Feasibility Jump runs after NoRelRepair as the general LP-free repair pass.
