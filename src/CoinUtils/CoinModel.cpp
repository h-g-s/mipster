...
/* MIPster no longer links against the AMPL Solver Library (ASL); the
 * AMPL/.nl-file reading support that used to live here (condensed from
 * Clp_ampl.cpp / Cbc_ampl.cpp) has been removed. The functions below are
 * kept as no-op stubs so callers built around the ampl_info API still link.
 */
int readAmpl(ampl_info *, int, char **, void **, const char *)
{
  return 0;
}
