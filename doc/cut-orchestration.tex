...
\subsection*{Ranking site 1: violation sort of the global pool}
...
\texttt{CbcModel.cpp:9101} sorts those violated cuts by violation
(descending) and applies a budget:
...
So the rule is: \emph{most-violated first, until the matrix-element
budget is exhausted}.

\subsection*{Ranking site 2: dive conflict cuts}

\texttt{doHeuristicsAtRoot} (the new MIPster path,
\texttt{CbcModel.cpp:16800--16846}) collects conflict cuts from the
\texttt{CbcRootHeuristicSchedule} dives.  It scores each by
\emph{normalised violation per element} and keeps the top
\texttt{conflictMaxCuts()}:
...
\texttt{CbcSolver.cpp:9235--9286}: Probing, Gomory, KnapsackCover,
RedSplit, Clique~(BK), OddWheel, MIR2, FlowCover, Twomir.  Primal
...
(\texttt{CbcModel.cpp:9066--9131}), each saved cut is admitted only
if its violation against the current $x^\ast$ exceeds a fixed
threshold:

\begin{lstlisting}
double violation = thisCut->violated(cbcColSolution_);
if (violation > 0.005) {            // hard-coded in CbcModel.cpp:9096
  violations[numberPossible] = -violation;
  which[numberPossible++] = i;
}
\end{lstlisting}

So the cutoff for the global-pool re-injection is
\textbf{$0.005$ absolute violation} (a conservative multiple of the
LP primal tolerance, $\sim 10^{-7}$).  Cuts with smaller violation
...
\midrule
\endhead
Global-pool violation cutoff   & \texttt{CbcModel.cpp:9096}        & $0.005$ (hard-coded)\\
Global-pool element budget     & \texttt{CbcModel.cpp:9075}        & $\max(\text{nz}/10,\,2n)+100$\\
Slack-cut re-admit cutoff      & \texttt{CbcModel.cpp:11109}       & $100\,\tau_{\text{prim}}$\\
\texttt{switchOffIfLessThan\_} & \texttt{CbcCutGenerator.cpp:1567} & $0$ per generator\\
\texttt{maximumTries\_}        & \texttt{CbcCutGenerator.cpp:1575} & $-1$ (off)\\
Gomory cut-length cap          & \texttt{CbcSolverCutSetup.cpp:175}& $-1$ (off)\\
Dive conflict-cut top-$K$      & \texttt{CbcModel.cpp:9994}        & $K=128$\\
...
The two main places in the codebase where \texttt{CoinCutPool}
would replace ad-hoc top-$K$ logic, ranked by expected impact:

\begin{enumerate}[leftmargin=*]
  \item \textbf{Dive conflict cuts} (\texttt{CbcModel.cpp:9994--10008}
        and \texttt{16800--16846}).  Currently sorts by
        $(lhs-ub)/\text{nz}$ and keeps top \texttt{conflictMaxCuts\_}
        (\S\ref{sec:thresholds}, item~6).  This is \emph{exactly}
        the niche \texttt{CoinCutPool} occupies, but with the
        added best-by-variable coverage that the current sort lacks.
  ...
  \item \textbf{Global-pool re-scan in \texttt{solveWithCuts}}
        (\texttt{CbcModel.cpp:9066--9131}).  The current logic
        sorts by violation only.  A \texttt{CoinCutPool} layer
        would add coverage, at the cost of $O(n_{\text{cols}})$
        memory per scan.
...
\toprule
\textbf{Generator} & \textbf{Key limits set} & \textbf{Accuracy flag}\\
\midrule
\endhead
Probing & \texttt{maxPass=1, maxProbe=123, maxLook=10/20,
  maxElements=200/300, rowCuts=$-3$} & 5\\
Gomory  & \texttt{limitAtRoot=1000, limit=50, awayAtRoot=0.005}
  & 3\\
Knapsack & (no special limits) & 1\\
MIR/Flow & (default params) & 2\\
TwoMIR  & \texttt{maxElements=250, awayAtRoot=0.005, away=0.01}
  & 4\\
Clique  & (CoinCutPool-based selection inside) & 0\\
OddWheel& (CoinCutPool-based selection inside) & 0\\
...
\midrule
\endhead
Global-pool violation cutoff & 0.005 & CbcModel.cpp:9096 &
  Minimum violation for re-scan; tighter $\Rightarrow$ tree
  benefits more from root cuts.\\
Global-pool element budget & $\max(\text{nz}/10,2n)+100$ &
  CbcModel.cpp:9075 & Maximum NZ the LP can grow per global-pool
  injection.\\
