...
build_variant() {
  ...
  # Set RPATH to $ORIGIN/../lib/<variant> so libmipster.so + bundled deps are
  # found relative to the binary on the user's machine. Without this the
  # binary retains its build-time RUNPATH (/tmp/mipster-build-*/install/lib)
  # and fails to start anywhere except the build container.
  ...
  # ── Bundle non-system runtime deps ──────────────────────────────────────────
  # Copy any NEEDED .so that isn't a baseline glibc/libm/libpthread/libdl/libz
  # into lib/<variant>/ so the wheel works on minimal containers (Alpine
  # base + glibc, slim Debian, etc.). All bundled libs must permit
  # redistribution; we currently expect libgfortran (GCC runtime exception),
  # libquadmath (same), libgomp (same), libbz2 (BSD).
  local main_so
  ...
  # Verify no remaining non-bundled non-system deps after RPATH lookup
  ...
  # Sanity: assert no /tmp/* in RUNPATH of any installed binary or .so
